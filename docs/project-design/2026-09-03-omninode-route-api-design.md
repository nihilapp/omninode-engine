...
1. 같은 Nuxt 서버에서 제공하는 웹앱 인증은 접근·리프레시 자격증명을 `HttpOnly` 쿠키로만 보관한다. 접근 자격증명은 서명된 최대 1시간 토큰이고, 브라우저별 리프레시 자격증명은 원문을 서버에 저장하지 않는 불투명 비밀값이다.
2. 리프레시 자격증명의 유효 기간은 로그인 성공 시점부터 고정 7일이며, 접근 토큰 재발급과 세션 점검은 이 만료 시각을 연장하지 않는다. 접근 토큰의 만료 시각도 전역 로그인 세션 만료 시각을 넘지 않는다.
3. 서버는 모든 관리 API 요청과 리프레시 요청마다 접근 자격증명, 전역 로그인 세션, 현재 역할, 월드·프로젝트 범위, 세부 허가를 다시 검사한다. 로그인 시점의 권한이나 1분 세션 점검 결과를 고정하지 않는다.
4. `GET /api/v1/auth/sessions/current`은 유효 세션을 확인하고 성공했을 때 마지막 접속 시각을 기록한다. 클라이언트는 로그인 상태에서 1분 주기로 이 확인을 수행하되, 실패 뒤 즉시 재시도하지 않는다. 이 주기 점검은 즉시 접근 차단의 근거가 아니라 상태 표기와 마지막 접속 기록을 위한 보조 흐름이다.
5. 전역 로그아웃과 탈퇴 확정은 계정의 전역 로그인 세션을 종료하므로, 연결된 모든 기기·브라우저의 다음 접근과 리프레시를 함께 거부한다. 다른 기기 로그인도 전역 만료 시각을 연장하지 않는다.
6. 쿠키는 운영 환경에서 `Secure`와 적절한 `SameSite` 속성을 사용하고, 상태를 바꾸는 요청은 출처 검증으로 교차 사이트 요청을 차단한다. 토큰 원문은 API 응답과 로그에 다시 노출하지 않는다.
7. 미인증 관리 페이지 접근은 `/signin?returnTo=...`로 이동한다.
8. 임시 비밀번호 계정은 `/password/change`와 필요한 인증 API 외의 관리 접근을 허용하지 않는다.
9. 권한 변경으로 현재 페이지 접근을 잃으면 다음 API 요청을 거부하고 `/admin`의 접근 가능한 범위로 안내한다.
10. 기존 계정의 초대 수락은 초대 이메일과 같은 계정으로 로그인한 상태에서만 허용한다.
11. 공개 페이지는 로그인 여부와 무관하게 같은 공개본을 보여 준다. 관리자에게 관리 중인 현재 리비전을 대신 노출하지 않는다.
12. 비밀번호 재설정과 탈퇴 링크 재발급 요청은 계정 존재 여부를 구분하지 않는 같은 성공 응답을 사용한다.
...
| GET | `/api/v1/auth/sessions/current` | 현재 전역 로그인 세션 확인과 마지막 접속 시각 기록 |
| DELETE | `/api/v1/auth/sessions/current` | 계정의 전역 로그인 세션 종료와 모든 브라우저 로그아웃 |
| POST | `/api/v1/auth/sessions/refreshes` | 유효한 브라우저별 리프레시 자격증명으로 접근 토큰 재발급 |
